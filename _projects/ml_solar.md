...
Using this technique, the network was able to learn the relationship between Ca II K images and magnetograms and could generate realistic magnetograms given a random Ca II K image.

### Handling polarity

Because Ca II K images capture only the unsigned magnetic field, they carry no information about **polarity** (the sign of the magnetic field). This has to be reconstructed separately, using **Hale's polarity law**: the leading polarity of active regions (the polarity on the side facing the direction of solar rotation) reverses from one solar cycle to the next. Polarity is therefore assigned systematically based on which solar cycle a given observation belongs to.

## Results

The generated pseudo-magnetograms reproduce the large-scale distribution of magnetic flux across the solar disk with good qualitative agreement to real magnetograms. Performance is strongest in regions of strong magnetic field; the network is comparatively weaker in quiet-Sun regions, where the Ca II K signal is faint and its correspondence with the true magnetic field becomes less direct.


<div class="row justify-content-sm-center">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/project_imgs/caltomagnew.png" title="caltomag" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class='caption'>
    Comparison between the real MDI magnetogram and the generated pseudo-magnetogram given a Ca II K input image.
</div>


## Why this matters

The real value of this approach lies in the **temporal reach** it unlocks. Direct magnetogram observations only became available from the 1970s onward, while the KSO Ca II K archive extends back to 1904 — spanning several additional solar cycles. By learning a reliable mapping from Ca II K images to magnetograms, this technique offers a path to extending the Sun's usable magnetic record by decades, providing a substantially longer baseline for studying the solar dynamo and long-term solar magnetic activity.

More broadly, this project is an example of using machine learning not simply as a data-analysis tool, but as a way to **bridge an observational gap** — recovering physical information from an archival dataset that was never originally designed to provide it.
