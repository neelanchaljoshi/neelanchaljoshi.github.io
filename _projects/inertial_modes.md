...
To identify these modes, we look for peaks in the power spectrum at frequencies and azimuthal wavenumbers consistent with theoretical predictions. Each mode is characterised by its azimuthal order $$m$$, which describes how many oscillation cycles wrap around the sun in longitude, and its frequency $$\omega$$, which describes how fast the pattern rotates. When a peak is identified, we can extract the spatial structure of the mode — the eigenfunction — which describes how the velocity field associated with that mode is distributed over the solar surface and with depth.

These eigenfunctions carry rich physical information. Their precise shape is sensitive to the internal rotation profile of the sun, the strength of turbulent diffusion, and the redistribution of angular momentum by Reynolds stresses. By comparing the observed eigenfunctions against those predicted by theoretical models, we can constrain these physical quantities and test our understanding of solar dynamics.
